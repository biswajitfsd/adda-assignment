$(document).ready(function() {
	...
	function Booking(obj) {
		...
		var cost = 0,
			j = obj.startTime,
			timeInterval = obj.endTime - obj.startTime;
	...
	$(document).on(
		'change',
		'.startTime, .endTime .facilitiesOptions',
		function() {
		...
		if (endTimeVar !== '' && endTimeVar !== null) {
			...
			if (data.facilitie !== '') {
				...
				$('form')
					...
					.remove();
				$('form').prepend(
					'<div class="form-group priceDiv"><label for="price">Total Price:</label><div>Rs ' +
						priceObj.price +
						'</div></div>'
				);
				$('.formSubmit').removeAttr('disabled');
